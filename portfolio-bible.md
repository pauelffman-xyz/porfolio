...
*Agosto 2026 — ✅ movida y fusionada a pedido (dos iteraciones)*
...
- El id `#hsBridge` sigue siendo único en el documento — el JS (`IntersectionObserver` sobre `#hsGrid`) sigue funcionando igual porque solo dispara `.is-live` sobre `#hsBridge` vía `getElementById`, sin importar dónde esté en el DOM.

**Regla para el futuro:** si se vuelve a mover o separar, no duplicar el `id="hsBridge"` — debe existir una sola vez en el documento. Y si se necesita otra card "elevada" en el About Me, reusar `--surface`/`--shadow-card` (mismo criterio que `.ai-lab` y `.hs-feature`) en vez de inventar un tercer tratamiento de fondo.
...
**JS (IIFE autocontenido sobre** `#hsGrid`**):** un `IntersectionObserver` dispara el reveal de las cells y el `.is-live` del bridge cuando la sección entra en viewport; los `transition-delay` inline se limpian tras el reveal para que el hover no quede retrasado. Respeta `prefers-reduced-motion` (todo en estado final, sin animación).
